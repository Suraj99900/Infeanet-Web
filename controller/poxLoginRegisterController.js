$(document).ready(() => {
    ...
    document.getElementById("userTypeId").addEventListener("change", function () {
        ...
        let keyDiv = document.getElementById("secretKeyWrapper");

        if (selectedType == "1") {
            keyDiv.style.display = "block";     // Show only for Admin
        } else {
            keyDiv.style.display = "none";      // Hide for Student or empty
            document.getElementById("KeyId").value = ""; // Clear key
        }
    });
