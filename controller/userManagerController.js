$(document).ready(() => {
    ...
    // Enable Select2
    // $('.select2').select2({
    //     dropdownParent: $('#AddUserOffCanvasId, #UpdateUserOffCanvasId')
    // });

    // Load semesters
    function loadSemesters(selectBoxId) {
        $.ajax({
            url: "ajaxFile/ajaxSemester.php",
            type: "POST",
            data: { sFlag: "fetchAll" },
            dataType: "json",
            success: function (response) {
                ...
                response.data.forEach(function (item) {
                    html += `<option value="${item.id}">${item.semester}</option>`;
                });
    ...
    loadSemesters('#semesterId');
    loadSemesters('#updateSemesterId');

    // Show/Hide Semester Dropdown
    $("#userType").on("change", function () {
        if ($(this).val() == "2") {
            $("#semesterBox").removeClass("d-none");
        } else {
            $("#semesterBox").addClass("d-none");
            $("#semesterId").val('').trigger('change');
        }
    });

    $("#updateUserType").on("change", function () {
        if ($(this).val() == "2") {
            $("#updateSemesterBox").removeClass("d-none");
        } else {
            $("#updateSemesterBox").addClass("d-none");
            $("#updateSemesterId").val('').trigger('change');
        }
    });
